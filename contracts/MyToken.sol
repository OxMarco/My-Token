...
contract MyToken is Context, IERC20, Ownable {
    ...
    address payable private _marketingAddress = payable(address(0)); //< CHANGEME
        ...
        returns (uint256)
    {
        require(
            rAmount <= _rTotal,
            "TKN: Amount must be less than total reflections"
        );
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner()) {
            ...
            // Trade start check
            if (!tradingOpen) {
                require(preTrader[from], "TKN: Cannot send tokens until trading is enabled");
            }
              
            require(amount <= _maxTxAmount, "TKN: Max transaction limit exceeded");
            require(!bots[from] && !bots[to], "TKN: Your account is blacklisted!");
            
            if(to != uniswapV2Pair) {
                require(balanceOf(to) + amount < _maxWalletSize, "TKN: Balance exceeds max wallet size!");
            }
    ...
    function allowPreTrading(address account, bool allowed) external onlyOwner {
        require(preTrader[account] != allowed, "TKN: Already set.");
